...
- (void)setGrade:(float)grade
{
    ...
    [self addAnimationIfNeededWithProgressLine:progressline];
     ...
     _chartLine.strokeColor = _barColor? [_barColor CGColor]:[PNGreen CGColor];
...
- (void)setIsNegative:(BOOL)isNegative
{
    if (isNegative)
    {
        ...
        CGSize size = CGSizeMake(200,20); //设置一个行高上限
...
-(void)setGradeFrame:(CGFloat)grade startPosY:(CGFloat)startPosY
{
    ...
    NSString *text = [[NSString alloc]initWithFormat:@"%0.f",grade*self.maxDivisor];
    ...
    //self.textLayer.wrapped = YES;//自适应
    [self.textLayer setString:text];
    [self.textLayer setFrame:CGRectMake((textWidth - size.width)/2.0,-size.height, size.width,size.height)];
