...
@property (nonatomic) UILabel                     *centerdescripL;
...
- (void)strokeChart
{
    ...
    if(self.showCenterdescription)
    {
        if(_centerdescripL == nil)
        _centerdescripL        = [[UILabel alloc]init];
        CGFloat width          = _innerCircleRadius * 2;
        _centerdescripL.frame  = CGRectMake(0, 0, width, width);
        _centerdescripL.center = _contentView.center;
        _centerdescripL.numberOfLines = 0;
        _centerdescripL.textColor = [UIColor darkGrayColor];
        _centerdescripL.textAlignment = NSTextAlignmentCenter;
        if(_centerdesString)
           _centerdescripL.text   = _centerdesString;
        else
