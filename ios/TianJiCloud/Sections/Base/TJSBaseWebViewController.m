...
@implementation TJSBaseWebViewController
...
#pragma mark - <TJSBaseWebControllerProtocol>

- (WKWebView *)tjs_wkWebView{
...
- (void)tjs_updateConstraintsWithEdgeInsets:(UIEdgeInsets)edgeInset {
    ...
    [self.wkWebView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.edges.mas_equalTo(edgeInset);
    }];
}
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(null_unspecified WKNavigation *)navigation {
    ...
    if(self.params[@""]) [self.wkWebView.scrollView.mj_header endRefreshing];
}
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([requestString hasPrefix:@""] &&
        ...
        ![requestString isEqualToString:_finalUrl] && _finalUrl) {
...
initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(NSString * _Nullable))completionHandler{
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"estimatedProgress"]) {
        ...
        if (object == _wkWebView) {
            ...
            self.navigationItem.title = _titleName ?: (_wkWebView.title ?: @"");
        }else{
...
- (void)createWkWebview {
    ...
    self.wkWebView = ({
        ...
        TJSWeakScriptMessageHandler *headler = [[TJSWeakScriptMessageHandler alloc] init];
        headler.scriptMessageHandler = self;
        // 将UserContentController设置到配置文件中
        [userContent addScriptMessageHandler:headler name:@""];
    ...
    self.requestProgressView = ({
        ...
        progressView.progressTintColor = nil;//ThemeService.main_color_00;
        progressView.trackTintColor    = nil;//ThemeService.weak_color_13;
...
- (void)dealloc {
    ...
    [_wkWebView.configuration.userContentController removeScriptMessageHandlerForName:@""];
