...
@interface TJSAppServiceManager ()

@property (nonatomic, strong) Class<UIApplicationDelegate> appDelegateClass;
@property (nonatomic, strong) NSMutableDictionary<NSString*, id<TJSAppService>> *servicesMap;

@end

// 由子类实现服务代理方法，父类负责把这些实现添加到 ApplicationDelegate 中
// 这些都需要在 ApplicationDelegate 实例创建之前执行
@implementation TJSAppServiceManager

+ (instancetype)sharedManager {
    static TJSAppServiceManager *_sharedManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedManager = [TJSAppServiceManager new];
    });
...
- (void)registerService:(id<TJSAppService>)service {
    ...
    id<TJSAppService> pre = self.servicesMap[[service serviceName]];
    if (pre) {
...
- (BOOL)proxyCanResponseToSelector:(SEL)aSelector {
    ...
    [self.servicesMap enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, id<TJSAppService> _Nonnull obj, BOOL * _Nonnull stop) {
